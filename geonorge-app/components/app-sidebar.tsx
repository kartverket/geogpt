...
export function AppSidebar({
  ...
  const handlePrevTheme = () => {
    setCurrentThemeIndex((prev) =>
      prev === 0 ? mapThemes.length - 1 : prev - 1
    );
    setSelectedBaseMap(
      mapThemes[
        currentThemeIndex === 0 ? mapThemes.length - 1 : currentThemeIndex - 1
      ].id
    );
    const themeFunction =
      mapThemes[
    ...
    if (themeFunction) themeFunction();
  };

  const handleNextTheme = () => {
    setCurrentThemeIndex((prev) =>
      prev === mapThemes.length - 1 ? 0 : prev + 1
    );
    setSelectedBaseMap(
      mapThemes[
        currentThemeIndex === mapThemes.length - 1 ? 0 : currentThemeIndex + 1
      ].id
    );
    const themeFunction =
      mapThemes[
