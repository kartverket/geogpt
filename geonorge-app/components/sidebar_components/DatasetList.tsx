...
export const DatasetList: React.FC<DatasetListProps> = ({
    ...
    const handleToggleDatasetExpansion = (datasetId: string) => {
        ...
        if (expandedDatasets[datasetId] && datasetScrollContainerRef.current) {
            const datasetElement = datasetScrollContainerRef.current.querySelector(`[data-dataset-id="${datasetId}"]`);
            if (datasetElement) {
    ...
    return (
        <div className="space-y-2 max-h-[30vh] overflow-y-auto" ref={datasetScrollContainerRef}>
            {trackedDatasets.map((dataset) => (
                <div key={dataset.id} className="border border-gray-200 rounded-md bg-white overflow-hidden" data-dataset-id={dataset.id}>
                    <div className="flex items-center justify-between p-2.5 bg-gray-50 cursor-pointer border-b border-gray-100" onClick={() => handleToggleDatasetExpansion(dataset.id)}>
                ...
                {dataset.selectedLayers.length}/{dataset.availableLayers.length}
                        ...
                        <div className="flex items-center gap-1.5">
                            ...
                            {expandedDatasets[dataset.id] ? <ChevronUp className="h-4 w-4 text-gray-500" /> : <ChevronDown className="h-4 w-4 text-gray-500" />}
                        </div>
                    </div>
                    ...
                    {expandedDatasets[dataset.id] && (
                        <div className="max-h-[20vh] overflow-y-auto dataset-layer-container">
                            {dataset.availableLayers.map((layer) => (
                                <div key={`${dataset.id}-${layer.name}`} className="flex items-center gap-2.5 p-2.5 hover:bg-gray-50 transition-colors border-t border-gray-100 first:border-t-0">
                                    <Checkbox
                                        checked={dataset.selectedLayers.includes(layer.name)}
                                        id={`${dataset.id}-${layer.name}`}
                                        onCheckedChange={(checked) => handleLayerChange(dataset.id, layer.name, checked as boolean)}
                                        className="h-4 w-4 border-gray-300 rounded"
                                    />
                                    <label htmlFor={`${dataset.id}-${layer.name}`} className="text-sm cursor-pointer flex-1 truncate text-gray-700">
                                        {layer.title}
                                    </label>
                                </div>
                            ))}
