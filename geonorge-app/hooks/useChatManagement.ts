...
interface ChatManagementProps {
  ...
  replaceIframe: (dataset: any) => void;
}
...
export const useChatManagement = ({
  ...
  // Handle keyboard events for both popover and fullscreen modes
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // For popover mode - close on ESC if popover is open and not blocked
      if (e.key === "Escape" && isPopoverOpen && !popoverBlocker.isBlocked) {
  ...
  }, [isFullScreen, isPopoverOpen, popoverBlocker.isBlocked]);
  ...
  // This function toggles the popover state without being affected by outside clicks
  const togglePopover = () => {
